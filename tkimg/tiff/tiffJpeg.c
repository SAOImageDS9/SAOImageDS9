...
TIFFjpeg_tables_dest(JPEGState* sp, TIFF* tif)
{
	...
	sp->jpegtables_length = 1000;
	sp->jpegtables = (void*) TkimgTIFFmalloc((tmsize_t) sp->jpegtables_length);
	...
	if (nrows)
        {
                ...
                if( sp->cinfo.d.data_precision == 12 )
                {
                        line_work_buf = (JSAMPROW)
                                TkimgTIFFmalloc(sizeof(short) * sp->cinfo.d.output_width
                                            * sp->cinfo.d.num_components );
                }
...
JPEGDecodeRaw(TIFF* tif, uint8* buf, tmsize_t cc, uint16 s)
{
	...
	if ( nrows != 0 ) {
		...
		unsigned short* tmpbuf = TkimgTIFFmalloc(sizeof(unsigned short) *
...
int
TkimgTIFFInitJpeg(TIFF* tif, int scheme)
{
	JPEGState* sp;
	 ...
	 */
	tif->tif_data = (uint8*) TkimgTIFFmalloc(sizeof (JPEGState));
...
*/
            ...
            sp->jpegtables = (void *) TkimgTIFFmalloc(sp->jpegtables_length);
            if (sp->jpegtables)
            {
