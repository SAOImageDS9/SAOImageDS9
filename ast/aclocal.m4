...
# and having "GNU Fortran * 4.x[x].x[x]" (or 1x.x[x].x[x]) as part of
# its --version output.
#
...
#
# For gfortran, version 8.1 and onwards use size_t. Earlier versions use
# int.
#
# For most other compilers the maximum length of a string is limited to a 32bit
...
AC_DEFUN([STAR_CNF_TRAIL_TYPE],
   [AC_CHECK_SIZEOF(void*)dnl
    ...
    AC_CACHE_CHECK([type used for Fortran string lengths],
       ...
       [if test "$ac_cv_fc_compiler_gnu" = yes; then
           if "$FC" --version 2>&1 < /dev/null | grep 'GNU Fortran.*[[8-9]]\.[[0-9]][[0-9]]*\.[[0-9]][[0-9]]*' > /dev/null; then
...
dnl  sizeof(void *) != 8 and not GNU so assume no problems.
           star_cv_cnf_trail_type=int
        fi
])
