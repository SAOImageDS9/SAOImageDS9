...
#   -- from the flex(1) man page
proc xpafirst::yyrestart {new_file} {
    variable yyin
...
#   -- from the flex(1) man page
proc xpafirst::YY_INPUT {buf result max_size} {
    variable yyin
...
#   -- from the flex(1) man page
proc xpafirst::yy_scan_string {str} {
    variable yy_current_buffer
    variable yyin
...
set UNIX_ 272
    ...
    while {1} {
        ...
        # rule 3: inet
        if {[regexp -start $index_ -indices -line -nocase -- {\A(inet)} $yy_current_buffer match] > 0 && \
                [lindex $match 1] - $index_ + 1 > $yyleng} {
            set yytext [string range $yy_current_buffer $index_ [lindex $match 1]]
            set yyleng [string length $yytext]
            set matched_rule 3
        ...
        # rule 6: localhost
        if {[regexp -start $index_ -indices -line -nocase -- {\A(localhost)} $yy_current_buffer match] > 0 && \
                [lindex $match 1] - $index_ + 1 > $yyleng} {
            set yytext [string range $yy_current_buffer $index_ [lindex $match 1]]
            set yyleng [string length $yytext]
            set matched_rule 6
        ...
        # rule 7: noxpans
        if {[regexp -start $index_ -indices -line -nocase -- {\A(noxpans)} $yy_current_buffer match] > 0 && \
                [lindex $match 1] - $index_ + 1 > $yyleng} {
            set yytext [string range $yy_current_buffer $index_ [lindex $match 1]]
            set yyleng [string length $yytext]
            set matched_rule 7
        ...
        # rule 8: tcl
        if {[regexp -start $index_ -indices -line -nocase -- {\A(tcl)} $yy_current_buffer match] > 0 && \
                [lindex $match 1] - $index_ + 1 > $yyleng} {
            set yytext [string range $yy_current_buffer $index_ [lindex $match 1]]
            set yyleng [string length $yytext]
            set matched_rule 8
        ...
        # rule 9: unix
        if {[regexp -start $index_ -indices -line -nocase -- {\A(unix)} $yy_current_buffer match] > 0 && \
                [lindex $match 1] - $index_ + 1 > $yyleng} {
            set yytext [string range $yy_current_buffer $index_ [lindex $match 1]]
            set yyleng [string length $yytext]
            set matched_rule 9
