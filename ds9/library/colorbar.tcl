...
proc ResetColormap {} {
    ...
    if {$current(frame) != {} } {
	EvalLockCurrent rgb(lock,colorbar) [list $current(frame) colormap [$current(colorbar) get colormap]]
    }
...
proc BeginAdjustColormap {} {
    ...
    if {$current(frame) != {}} {
	EvalLockCurrent rgb(lock,colorbar) [list $current(frame) colormap begin]
    }
}
...
proc AdjustColormap {} {
	...
	EvalLockColorbarCurrent [list $current(colorbar) adjust $dcolorbar(contrast) $dcolorbar(bias)]
	if {$current(frame) != {}} {
	    EvalLockCurrent rgb(lock,colorbar) [list $current(frame) colormap motion [$current(colorbar) get colormap]]
	}
    }
}
...
proc EndAdjustColormap {} {
	...
	if {$current(frame) != {}} {
	    EvalLockCurrent rgb(lock,colorbar) [list $current(frame) colormap end]
	    LockColorCurrent
	}
    }
}
