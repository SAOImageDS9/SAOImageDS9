...
#   -- from the flex(1) man page
proc web::YY_INPUT {buf result max_size} {
    variable yyin
...
set STOP_ 266
    ...
    while {1} {
        ...
        # rule 2: click
        if {[regexp -start $index_ -indices -line -nocase -- {\A(click)} $yy_current_buffer match] > 0 && \
                [lindex $match 1] - $index_ + 1 > $yyleng} {
            set yytext [string range $yy_current_buffer $index_ [lindex $match 1]]
            set yyleng [string length $yytext]
            set matched_rule 2
        ...
        # rule 6: reload
        if {[regexp -start $index_ -indices -line -nocase -- {\A(reload)} $yy_current_buffer match] > 0 && \
                [lindex $match 1] - $index_ + 1 > $yyleng} {
            set yytext [string range $yy_current_buffer $index_ [lindex $match 1]]
            set yyleng [string length $yytext]
            set matched_rule 6
