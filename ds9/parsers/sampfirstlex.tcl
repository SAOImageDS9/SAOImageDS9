...
#   -- from the flex(1) man page
proc sampfirst::YY_FLUSH_BUFFER {} {
    variable yy_current_buffer
    variable index_
    variable done_
...
#   -- from the flex(1) man page
proc sampfirst::yyrestart {new_file} {
    variable yyin
...
#   -- from the flex(1) man page
proc sampfirst::YY_INPUT {buf result max_size} {
    variable yyin
...
#   -- from the flex(1) man page
proc sampfirst::yy_scan_string {str} {
    variable yy_current_buffer
    variable yyin
...
set WEB_ 275
    ...
    while {1} {
        ...
        # rule 1: client
        if {[regexp -start $index_ -indices -line -nocase -- {\A(client)} $yy_current_buffer match] > 0 && \
                [lindex $match 1] - $index_ + 1 > $yyleng} {
            set yytext [string range $yy_current_buffer $index_ [lindex $match 1]]
            set yyleng [string length $yytext]
            set matched_rule 1
        ...
        # rule 3: disconnect
        if {[regexp -start $index_ -indices -line -nocase -- {\A(disconnect)} $yy_current_buffer match] > 0 && \
                [lindex $match 1] - $index_ + 1 > $yyleng} {
            set yytext [string range $yy_current_buffer $index_ [lindex $match 1]]
            set yyleng [string length $yytext]
            set matched_rule 3
        ...
        # rule 4: hub
        if {[regexp -start $index_ -indices -line -nocase -- {\A(hub)} $yy_current_buffer match] > 0 && \
                [lindex $match 1] - $index_ + 1 > $yyleng} {
            set yytext [string range $yy_current_buffer $index_ [lindex $match 1]]
            set yyleng [string length $yytext]
            set matched_rule 4
        ...
        # rule 7: send
        if {[regexp -start $index_ -indices -line -nocase -- {\A(send)} $yy_current_buffer match] > 0 && \
                [lindex $match 1] - $index_ + 1 > $yyleng} {
            set yytext [string range $yy_current_buffer $index_ [lindex $match 1]]
            set yyleng [string length $yytext]
            set matched_rule 7
        ...
        # rule 8: start
        if {[regexp -start $index_ -indices -line -nocase -- {\A(start)} $yy_current_buffer match] > 0 && \
                [lindex $match 1] - $index_ + 1 > $yyleng} {
            set yytext [string range $yy_current_buffer $index_ [lindex $match 1]]
            set yyleng [string length $yytext]
            set matched_rule 8
        ...
        # rule 11: web
        if {[regexp -start $index_ -indices -line -nocase -- {\A(web)} $yy_current_buffer match] > 0 && \
                [lindex $match 1] - $index_ + 1 > $yyleng} {
            set yytext [string range $yy_current_buffer $index_ [lindex $match 1]]
            set yyleng [string length $yytext]
            set matched_rule 11
