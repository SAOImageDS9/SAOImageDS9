...
#   -- from the flex(1) man page
proc bin::YY_INPUT {buf result max_size} {
    variable yyin
...
set TO_ 285
    ...
    while {1} {
        ...
        set matched_rule -1
        # rule 0: about
        if {[regexp -start $index_ -indices -line -nocase -- {\A(about)} $yy_current_buffer match] > 0 && \
        ...
        # rule 1: average
        if {[regexp -start $index_ -indices -line -nocase -- {\A(average)} $yy_current_buffer match] > 0 && \
                [lindex $match 1] - $index_ + 1 > $yyleng} {
            set yytext [string range $yy_current_buffer $index_ [lindex $match 1]]
            set yyleng [string length $yytext]
            set matched_rule 1
        ...
        # rule 2: buffersize
        if {[regexp -start $index_ -indices -line -nocase -- {\A(buffersize)} $yy_current_buffer match] > 0 && \
                [lindex $match 1] - $index_ + 1 > $yyleng} {
            set yytext [string range $yy_current_buffer $index_ [lindex $match 1]]
            set yyleng [string length $yytext]
            set matched_rule 2
        ...
        # rule 6: cols
        if {[regexp -start $index_ -indices -line -nocase -- {\A(cols)} $yy_current_buffer match] > 0 && \
                [lindex $match 1] - $index_ + 1 > $yyleng} {
            set yytext [string range $yy_current_buffer $index_ [lindex $match 1]]
            set yyleng [string length $yytext]
            set matched_rule 6
        ...
        # rule 7: colsz
        if {[regexp -start $index_ -indices -line -nocase -- {\A(colsz)} $yy_current_buffer match] > 0 && \
                [lindex $match 1] - $index_ + 1 > $yyleng} {
            set yytext [string range $yy_current_buffer $index_ [lindex $match 1]]
            set yyleng [string length $yytext]
            set matched_rule 7
        ...
        # rule 8: depth
        if {[regexp -start $index_ -indices -line -nocase -- {\A(depth)} $yy_current_buffer match] > 0 && \
                [lindex $match 1] - $index_ + 1 > $yyleng} {
            set yytext [string range $yy_current_buffer $index_ [lindex $match 1]]
            set yyleng [string length $yytext]
            set matched_rule 8
        ...
        # rule 9: factor
        if {[regexp -start $index_ -indices -line -nocase -- {\A(factor)} $yy_current_buffer match] > 0 && \
                [lindex $match 1] - $index_ + 1 > $yyleng} {
            set yytext [string range $yy_current_buffer $index_ [lindex $match 1]]
            set yyleng [string length $yytext]
            set matched_rule 9
        ...
        # rule 18: sum
        if {[regexp -start $index_ -indices -line -nocase -- {\A(sum)} $yy_current_buffer match] > 0 && \
                [lindex $match 1] - $index_ + 1 > $yyleng} {
            set yytext [string range $yy_current_buffer $index_ [lindex $match 1]]
            set yyleng [string length $yytext]
            set matched_rule 18
