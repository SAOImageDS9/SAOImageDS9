...
#include "util.h"
#include "colorbarhlstruecolor24.h"

// Tk Canvas Widget Function Declarations

int ColorbarHLSTrueColor24CreateProc(Tcl_Interp*, Tk_Canvas, Tk_Item*, int, 
				  Tcl_Obj *const []);
...
static Tk_ConfigSpec colorbarHLSTrueColor24Specs[] = {
...
static Tk_ItemType colorbarHLSTrueColor24Type = {
  (char*)"colorbarhlstruecolor24",        // name
  sizeof(ColorbarBaseOptions), // size
  ColorbarHLSTrueColor24CreateProc, // configProc
  colorbarHLSTrueColor24Specs,     // configSpecs
...
int ColorbarHLSTrueColor24_Init(Tcl_Interp* interp)
{
  tagsOption.parseProc = Tk_CanvasTagsParseProc;
  tagsOption.printProc = Tk_CanvasTagsPrintProc;

  Tk_CreateItemType(&colorbarHLSTrueColor24Type);
  return TCL_OK;
}

int ColorbarHLSTrueColor24CreateProc(Tcl_Interp* interp, Tk_Canvas canvas, 
				  Tk_Item* item, int argc, 
				  Tcl_Obj *const argv[])
{
  ColorbarHLSTrueColor24* colorbar = 
    new ColorbarHLSTrueColor24(interp,canvas,item);
...
void ColorbarHLSTrueColor24::updateColorsVert()
{
...
void ColorbarHLSTrueColor24::updateColors24Horz(int width, int height,
						char* data)
{
...
void ColorbarHLSTrueColor24::updateColors24Vert(int width, int height,
						char* data)
{
  // if we have cross platforms, we need to byte swap
  if ((!xmap->byte_order && lsb()) || (xmap->byte_order && !lsb())) {
...
void ColorbarHLSTrueColor24::updateColors32Horz(int width, int height, 
						char* data)
{
  // if we have cross platforms, we need to byte swap
  unsigned char row[xmap->bytes_per_line];
...
void ColorbarHLSTrueColor24::updateColors32Vert(int width, int height, 
						char* data)
{
  // if we have cross platforms, we need to byte swap
  if ((!xmap->byte_order && lsb()) || (xmap->byte_order && !lsb())) {
