...
#   -- from the flex(1) man page
proc hlssend::yyrestart {new_file} {
    variable yyin
...
#   -- from the flex(1) man page
proc hlssend::YY_INPUT {buf result max_size} {
    variable yyin
...
#   -- from the flex(1) man page
proc hlssend::yy_scan_string {str} {
    variable yy_current_buffer
    variable yyin
...
set WCS_ 274
    ...
    while {1} {
        ...
        # rule 3: channel
        if {[regexp -start $index_ -indices -line -nocase -- {\A(channel)} $yy_current_buffer match] > 0 && \
                [lindex $match 1] - $index_ + 1 > $yyleng} {
            set yytext [string range $yy_current_buffer $index_ [lindex $match 1]]
            set yyleng [string length $yytext]
            set matched_rule 3
