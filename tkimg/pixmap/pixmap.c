 ...
 * pixmap.c
 *
 * PIXMAP image type, Tcl/Tk package.
 *
 * A new image type for the PIXMAP image format.
 *
 * For a list of available format options see the documentation img-pixmap.
 *
...
TkimgXpmGetDataFromFile(
    ...
    chan = tkimg_OpenFileChannel(interp, fileName, "r");
    if (!chan) {
        return (const char **) NULL;
    }
