...
void Base::analysisMarkersInit()
{
  ...
  analysisMarkers.append(new Circle(this, cc, rr));
  analysisMarkers.append(new Box(this, cc, ss, ang));
  analysisMarkers.append(new Ellipse(this, cc, ss, ang));
  analysisMarkers.append(new Polygon(this, cc, ss));
  analysisMarkers.append(new Point(this, cc));
  analysisMarkers.append(new Vect(this, cc, mag, ang));
  analysisMarkers.append(new Annulus(this, cc, in, out, num));
  analysisMarkers.append(new EllipseAnnulus(this, cc, vin, vout, num, ang));
  analysisMarkers.append(new BoxAnnulus(this, cc, vin, vout, num, ang));

  Marker* mm = analysisMarkers.head();
  while (mm) {
    mm->setProperty(Marker::SELECT, 1);
    mm->setProperty(Marker::EDIT, 1);
    mm->setProperty(Marker::ROTATE, 1);
    mm = mm->next();
  }
}
*/

void Base::analysisMarkersResetCmd()
{
  /*
  if (analysisMarkers.isEmpty())
    analysisMarkersInit();

  Marker* mm = analysisMarkers.head();
  while (mm) {
    mm->setProperty(Marker::HIDDEN, 1);
    mm = mm->next();
  }
  */
}

void Base::analysisMarkersSelectCmd(const char* which, const Vector& cc)
{
  /*
  // reset
  analysisMarkersResetCmd();

  Marker* mm = analysisMarkers.head();
  while (mm) {
    if (!strcmp(mm->getType(),which)) {
      mm->setProperty(Marker::HIDDEN, 0);
      mm->moveTo(cc);
      update(PIXMAP);
      return;
    }
