...
#   -- from the flex(1) man page
proc vo::YY_INPUT {buf result max_size} {
    variable yyin
...
set XPA_ 274
    ...
    while {1} {
        ...
        # rule 2: delay
        if {[regexp -start $index_ -indices -line -nocase -- {\A(delay)} $yy_current_buffer match] > 0 && \
                [lindex $match 1] - $index_ + 1 > $yyleng} {
            set yytext [string range $yy_current_buffer $index_ [lindex $match 1]]
            set yyleng [string length $yytext]
            set matched_rule 2
        ...
        # rule 4: internal
        if {[regexp -start $index_ -indices -line -nocase -- {\A(internal)} $yy_current_buffer match] > 0 && \
                [lindex $match 1] - $index_ + 1 > $yyleng} {
            set yytext [string range $yy_current_buffer $index_ [lindex $match 1]]
            set yyleng [string length $yytext]
            set matched_rule 4
        ...
        # rule 6: mime
        if {[regexp -start $index_ -indices -line -nocase -- {\A(mime)} $yy_current_buffer match] > 0 && \
                [lindex $match 1] - $index_ + 1 > $yyleng} {
            set yytext [string range $yy_current_buffer $index_ [lindex $match 1]]
            set yyleng [string length $yytext]
            set matched_rule 6
        ...
        # rule 9: xpa
        if {[regexp -start $index_ -indices -line -nocase -- {\A(xpa)} $yy_current_buffer match] > 0 && \
                [lindex $match 1] - $index_ + 1 > $yyleng} {
            set yytext [string range $yy_current_buffer $index_ [lindex $match 1]]
            set yyleng [string length $yytext]
            set matched_rule 9
