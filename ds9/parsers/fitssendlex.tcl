...
#   -- from the flex(1) man page
proc fitssend::yyrestart {new_file} {
    variable yyin
...
#   -- from the flex(1) man page
proc fitssend::YY_INPUT {buf result max_size} {
    variable yyin
...
#   -- from the flex(1) man page
proc fitssend::yy_scan_string {str} {
    variable yy_current_buffer
    variable yyin
...
set WIDTH_ 309
    ...
    while {1} {
        ...
        set matched_rule -1
        # rule 0: bitpix
        if {[regexp -start $index_ -indices -line -nocase -- {\A(bitpix)} $yy_current_buffer match] > 0 && \
        ...
        # rule 1: count
        if {[regexp -start $index_ -indices -line -nocase -- {\A(count)} $yy_current_buffer match] > 0 && \
                [lindex $match 1] - $index_ + 1 > $yyleng} {
            set yytext [string range $yy_current_buffer $index_ [lindex $match 1]]
            set yyleng [string length $yytext]
            set matched_rule 1
        ...
        # rule 3: gz
        if {[regexp -start $index_ -indices -line -nocase -- {\A(gz)} $yy_current_buffer match] > 0 && \
                [lindex $match 1] - $index_ + 1 > $yyleng} {
            set yytext [string range $yy_current_buffer $index_ [lindex $match 1]]
            set yyleng [string length $yytext]
            set matched_rule 3
        ...
        # rule 5: height
        if {[regexp -start $index_ -indices -line -nocase -- {\A(height)} $yy_current_buffer match] > 0 && \
                [lindex $match 1] - $index_ + 1 > $yyleng} {
            set yytext [string range $yy_current_buffer $index_ [lindex $match 1]]
            set yyleng [string length $yytext]
            set matched_rule 5
        ...
        # rule 8: resample
        if {[regexp -start $index_ -indices -line -nocase -- {\A(resample)} $yy_current_buffer match] > 0 && \
                [lindex $match 1] - $index_ + 1 > $yyleng} {
            set yytext [string range $yy_current_buffer $index_ [lindex $match 1]]
            set yyleng [string length $yytext]
            set matched_rule 8
