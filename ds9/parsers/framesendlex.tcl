...
#   -- from the flex(1) man page
proc framesend::YY_FLUSH_BUFFER {} {
    variable yy_current_buffer
    variable index_
    variable done_
...
#   -- from the flex(1) man page
proc framesend::yyrestart {new_file} {
    variable yyin
...
#   -- from the flex(1) man page
proc framesend::YY_INPUT {buf result max_size} {
    variable yyin
...
#   -- from the flex(1) man page
proc framesend::yy_scan_string {str} {
    variable yy_current_buffer
    variable yyin
...
set UNDO_ 313
    ...
    while {1} {
        ...
        set matched_rule -1
        # rule 0: active
        if {[regexp -start $index_ -indices -line -nocase -- {\A(active)} $yy_current_buffer match] > 0 && \
        ...
        # rule 2: aux
        if {[regexp -start $index_ -indices -line -nocase -- {\A(aux)} $yy_current_buffer match] > 0 && \
                [lindex $match 1] - $index_ + 1 > $yyleng} {
            set yytext [string range $yy_current_buffer $index_ [lindex $match 1]]
            set yyleng [string length $yytext]
            set matched_rule 2
        ...
        # rule 4: celestial
        if {[regexp -start $index_ -indices -line -nocase -- {\A(celestial)} $yy_current_buffer match] > 0 && \
                [lindex $match 1] - $index_ + 1 > $yyleng} {
            set yytext [string range $yy_current_buffer $index_ [lindex $match 1]]
            set yyleng [string length $yytext]
            set matched_rule 4
        ...
        # rule 5: contour
        if {[regexp -start $index_ -indices -line -nocase -- {\A(contour)} $yy_current_buffer match] > 0 && \
                [lindex $match 1] - $index_ + 1 > $yyleng} {
            set yytext [string range $yy_current_buffer $index_ [lindex $match 1]]
            set yyleng [string length $yytext]
            set matched_rule 5
        ...
        # rule 6: cube
        if {[regexp -start $index_ -indices -line -nocase -- {\A(cube)} $yy_current_buffer match] > 0 && \
                [lindex $match 1] - $index_ + 1 > $yyleng} {
            set yytext [string range $yy_current_buffer $index_ [lindex $match 1]]
            set yyleng [string length $yytext]
            set matched_rule 6
        ...
        # rule 9: equatorial
        if {[regexp -start $index_ -indices -line -nocase -- {\A(equatorial)} $yy_current_buffer match] > 0 && \
                [lindex $match 1] - $index_ + 1 > $yyleng} {
            set yytext [string range $yy_current_buffer $index_ [lindex $match 1]]
            set yyleng [string length $yytext]
            set matched_rule 9
        ...
        # rule 15: iis
        if {[regexp -start $index_ -indices -line -nocase -- {\A(iis)} $yy_current_buffer match] > 0 && \
                [lindex $match 1] - $index_ + 1 > $yyleng} {
            set yytext [string range $yy_current_buffer $index_ [lindex $match 1]]
            set yyleng [string length $yytext]
            set matched_rule 15
        ...
        # rule 19: marker
        if {[regexp -start $index_ -indices -line -nocase -- {\A(marker)} $yy_current_buffer match] > 0 && \
                [lindex $match 1] - $index_ + 1 > $yyleng} {
            set yytext [string range $yy_current_buffer $index_ [lindex $match 1]]
            set yyleng [string length $yytext]
            set matched_rule 19
