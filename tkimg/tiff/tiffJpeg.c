...
static int TIFFjpeg_tables_dest(JPEGState *sp, TIFF *tif) {
  ...
  if (sp->otherSettings.jpegtables)
    TkimgTIFFfree(sp->otherSettings.jpegtables);
  sp->otherSettings.jpegtables_length = 1000;
  sp->otherSettings.jpegtables =
      (void *)TkimgTIFFmalloc((tmsize_t)sp->otherSettings.jpegtables_length);
...
static void JPEGFixupTagsSubsampling(TIFF *tif) {
  ...
  m.tif = tif;
  m.buffersize = 2048;
  m.buffer = TkimgTIFFmalloc(m.buffersize);
  if (m.buffer == NULL) {
  ...
  */
  ...
  if (nrows) {
    ...
    if (sp->cinfo.d.data_precision == 12) {
      line_work_buf =
          (JSAMPROW)TkimgTIFFmalloc(sizeof(short) * sp->cinfo.d.output_width *
                                sp->cinfo.d.num_components);
    }
...
#if defined(JPEG_LIB_MK1_OR_12BIT)
    tmpbuf = TkimgTIFFmalloc(sizeof(unsigned short) * sp->cinfo.d.output_width *
                         sp->cinfo.d.num_components);
    if (tmpbuf == NULL) {
...
static int JPEGEncode(TIFF *tif, uint8_t *buf, tmsize_t cc, uint16_t s) {
  ...
  if (sp->cinfo.c.data_precision == 12) {
    ...
    line16 = (short *)TkimgTIFFmalloc(sizeof(short) * line16_count);
    if (!line16) {
...
int TkimgTIFFInitJpeg(TIFF *tif, int scheme) {
  JPEGState *sp;
   ...
   */
  tif->tif_data = (uint8_t *)TkimgTIFFmalloc(sizeof(JPEGState));
...
#define SIZE_OF_JPEGTABLES 2000
    ...
    */
    ...
    sp->otherSettings.jpegtables =
        (void *)TkimgTIFFmalloc(sp->otherSettings.jpegtables_length);
    if (sp->otherSettings.jpegtables) {
