...
F77_SUBROUTINE(read_line)( CHARACTER(prompt), CHARACTER(line) 
                           TRAIL(prompt) TRAIL(line) ) {
   GENPTR_CHARACTER(prompt)
   GENPTR_CHARACTER(line)

   char *cline;
   char *cprompt = malloc( prompt_length );

   if( cprompt ) {
      cnfImprt( prompt, prompt_length, cprompt );

      cline = readline( cprompt );
      if( cline ) {
         cnfExprt( cline, line, line_length );
         add_history( cline );
         free( cline );
      } else {
         cnfExprt( "EXIT", line, line_length );
      }
      free( cprompt );
   }
}
