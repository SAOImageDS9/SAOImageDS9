...
set TGSSADR_ 334
    ...
    while {1} {
        ...
        # rule 8: ivoid
        if {[regexp -start $index_ -indices -line -nocase -- {\A(ivoid)} $yy_current_buffer match] > 0 && \
                [lindex $match 1] - $index_ + 1 > $yyleng} {
            set yytext [string range $yy_current_buffer $index_ [lindex $match 1]]
            set yyleng [string length $yytext]
            set matched_rule 8
        ...
        # rule 14: registry
        if {[regexp -start $index_ -indices -line -nocase -- {\A(registry)} $yy_current_buffer match] > 0 && \
                [lindex $match 1] - $index_ + 1 > $yyleng} {
            set yytext [string range $yy_current_buffer $index_ [lindex $match 1]]
            set yyleng [string length $yytext]
            set matched_rule 14
