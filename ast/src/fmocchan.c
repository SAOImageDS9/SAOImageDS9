...
*  Purpose:
*     Define a FORTRAN 77 interface to the AST MocChan class.
...
*     a public FORTRAN 77 interface to the MocChan class.
...
#include "mocchan.h"             /* C interface to the MocChan class */
...
F77_INTEGER_FUNCTION(ast_mocchan)( void (* SOURCE)(),
                                   void (* SINK)(),
                                   CHARACTER(OPTIONS),
                                   INTEGER(STATUS)
                                   TRAIL(OPTIONS) ) {
...
/* Change ',' to '\n' (see AST_SET in fobject.c for why). */
      ...
      RESULT = astP2I( astMocChanFor( source, astSourceWrap, sink, astSinkWrap,
                                      "%s", options ) );
      astFree( options );
   )
   return RESULT;
...
F77_LOGICAL_FUNCTION(ast_isamocchan)( INTEGER(THIS),
                                      INTEGER(STATUS) ) {
   GENPTR_INTEGER(THIS)
   F77_LOGICAL_TYPE(RESULT);

   astAt( "AST_ISAMOCCHAN", NULL, 0 );
   astWatchSTATUS(
      RESULT = astIsAMocChan( astI2P( *THIS ) ) ? F77_TRUE : F77_FALSE;
   )
   return RESULT;
}
