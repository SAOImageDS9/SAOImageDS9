...
#   -- from the flex(1) man page
proc cmap::YY_INPUT {buf result max_size} {
    variable yyin
...
set VALUE_ 276
    ...
    while {1} {
        ...
        # rule 9: tag
        if {[regexp -start $index_ -indices -line -nocase -- {\A(tag)} $yy_current_buffer match] > 0 && \
                [lindex $match 1] - $index_ + 1 > $yyleng} {
            set yytext [string range $yy_current_buffer $index_ [lindex $match 1]]
            set yyleng [string length $yytext]
            set matched_rule 9
