...
set VERTGRAPH_ 320
    ...
    while {1} {
        ...
        # rule 21: park
        if {[regexp -start $index_ -indices -line -nocase -- {\A(park)} $yy_current_buffer match] > 0 && \
                [lindex $match 1] - $index_ + 1 > $yyleng} {
            set yytext [string range $yy_current_buffer $index_ [lindex $match 1]]
            set yyleng [string length $yytext]
            set matched_rule 21
        ...
        # rule 23: south
        if {[regexp -start $index_ -indices -line -nocase -- {\A(south)} $yy_current_buffer match] > 0 && \
                [lindex $match 1] - $index_ + 1 > $yyleng} {
            set yytext [string range $yy_current_buffer $index_ [lindex $match 1]]
            set yyleng [string length $yytext]
            set matched_rule 23
